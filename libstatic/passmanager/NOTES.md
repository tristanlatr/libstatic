...
- Why a pass manager? 
  ...
  support dataclasses by your code analysis since the implicit methods will become explicit; 
  Or expand wildcard imports, which are a major pain for most static analysis tools.
...
Nevertheless it will be alway possible to fool the most advanced python static analyzers by
the use of `eval()`, `sys.modules` or other dynamic features of the language.


(note)
One of the important design decision regarding reusability and security is the separation of the IO and the analysis. 
In the context of potentially malcious piece of code, it is important that the framework can ensure it will onlt oever work
with what's been given by the developers. 
...
- Provide support for namespace packages in the finder.
...
- ``rope``: Rope runs static analysis on the whole project or on a single python file when requested. 
  This task can be very time consuming.
  The approach here, in opposit, is to provide a framework to optimize passes and run only the minimum required.
  Also rope has not been built for beeing used as a library
...
As a general rule, I've tried to keep the dependencies at the lowest possible while 
not over-complexifying the implementation. This is why the following dependencies
are required at the moment: 

- The pass manager infrastructure currently uses ``attrs``.  
- The finder uses ``typeshed_client``. This could be an optinal dependency. 
- The def-use chains analysis is provided by ``beniget``, 
  so many interesting analyses will require it;
  Including the evaluation of ``__all__`` variables, and others. 
  This could be an optinal dependency as well.

Usages
------

- Support ``pydoctor`` in its code analysis.
- Wildcard unroller tool.
- Toy Rust/C++ transpiler.
