...
_Pointer: TypeAlias = (
    "tuple[Forest,] | tuple[Forest, MTree] | tuple[Forest, MTree, AnyNode]"
)
"""
...
class Hook(Protocol):
    ...
    def __call__(
        ...
        passe: PassInstance,
        pointer: _Pointer,
        returned: TransformationReturn | AnalysisReturn,
    ) -> TransformationReturn | AnalysisReturn | None: ...
        ...
        def _yield_deps(c: PassLike) -> Iterator[PassLike]:
            ...
            yield from (
                ...
                for d in chain.from_iterable(
                    _yield_deps(dep) for dep in c.proto.get_dependencies()
                )
                if d not in seen
            )
...
class PassInstance:
    ...
    def _add_args(self, *args: Hashable, **kwargs: Hashable) -> PassInstance:
        ...
        if args_dict:
            return PassInstance(self.proto, FrozenDict({**self.args, **args_dict}))
        else:
            return self
...
class PassContext:
    ...
    @contextmanager
    def _push_pass(
        self, passe: PassInstance, pointer: _Pointer
...
class Runner:
    ...
    def do_pass(self, c: Connector) -> TransformationReturn | AnalysisReturn:
        ...
        if not isinstance(_res, dict):
            ...
            result: TransformationReturn | AnalysisReturn = dict(
                _res
...
class PassManager:
    ...
    def __init__(self, trees: Iterable[MTree] | None = None) -> None:
        ...
        self.hooks: dict[Literal["transformation", "analysis"], list[Hook]] = (
            defaultdict(list)
        )

        self._ctx = PassContext()
        self._runners = {
            _ANALYSIS: AnalysisRunner,
    ...
    def _prepare_element(
        self, element: tuple[str | Element, ...], runs_on: _ElemKind
    ) -> tuple[Element, ...]:
